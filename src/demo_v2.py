...
Formatting:
- Time metrics (*Seconds) printed as XmYsec, rounded to nearest second
- All other metrics rounded to nearest 0.1
"""
...
def fmt_value(metric: str, value: float) -> str:
    # Only change requested: total sleep time in hours+minutes.
...
def main() -> None:
    ...
    for metric, higher_is_better in METRICS:
        ...
        if avg is None:
            print(f"Your {metric_label(metric)} was {fmt_value(metric, v)}. (Not enough prior-week data to compare.)")
            continue

        verdict = compare(v, avg, higher_is_better)
        print(
            f"Your {metric_label(metric)} was {fmt_value(metric, v)}; "
            f"this is {verdict} the previous week average of {fmt_value(metric, avg)}."
        )
